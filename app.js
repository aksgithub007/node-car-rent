...
// const MongoDBUri = "mongodb+srv://akshayshejwal007:Akki94212bhagu@cluster0.t2p5oye.mongodb.net/car-rent"
...
const MongoDBUri = process.env.MONGODB_URI  // ← from .env
...
// ← csrf with cookie options (fixes Render issue)
app.use(csrf({
  cookie: {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
...
app.use((err, req, res, next) => {
  let csrfToken = ''
  try { csrfToken = req.csrfToken() } catch (_) {}
